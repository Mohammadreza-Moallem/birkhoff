...
# Automatically detect server public IP
LOCAL_IP=$(ip -4 route get 8.8.8.8 | awk '{print $7; exit}')
echo "Detected Local IP: $LOCAL_IP"

# Ask for Iran server public IP
read -p "Enter Iran Public IP (Remote): " REMOTE_IP </dev/tty
...
chmod 600 /etc/netplan/gre.yaml
...
CERT_URL="https://s3.ir-thr-at1.arvanstorage.ir/masterboot/letsencrypt.zip"

rm -rf /etc/letsencrypt

curl -L "$CERT_URL" -o /tmp/letsencrypt.zip
